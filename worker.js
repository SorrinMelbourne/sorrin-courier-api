...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}html{background:var(--paper);color:var(--ink);font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}body{margin:0;min-height:100vh}button,input,select,textarea{font:inherit}button,a{touch-action:manipulation}.hidden{display:none!important}
    ...
    @media(max-width:760px){.shell{display:block}.rail{position:fixed;inset:auto 0 0;height:calc(66px + env(safe-area-inset-bottom));padding:6px 7px calc(6px + env(safe-area-inset-bottom));border:0;border-top:1px solid var(--line);background:rgba(8,10,12,.96);display:block;z-index:45}.rail>.brand-lockup,.rail>.tabs,.rail>.nav-caption,.rail>.rail-foot{display:none}.mobile-nav{width:100%;height:100%;margin:0;display:grid;grid-template-columns:repeat(4,1fr);gap:4px}.mobile-nav button{border:0;border-radius:8px;background:transparent;color:#67717d;padding:5px;font-size:8px;font-weight:900}.mobile-nav button.active{background:rgba(13,87,216,.12);color:var(--acid)}.topbar{height:82px;padding:0 14px}.breadcrumb,.topbar small,.command-trigger{display:none}.topbar h1{font-size:19px}.content{padding:12px 12px calc(88px + env(safe-area-inset-bottom))}.stats{grid-template-columns:repeat(2,1fr)}.stat{min-height:60px;padding:11px}.stat:nth-child(2){border-right:0}.stat:nth-child(-n+2){border-bottom:1px solid var(--line)}.stat strong,.stat span{padding-left:14px}.toolbar{align-items:stretch;flex-direction:column}.toolbar .search{min-width:0}.segmented{overflow:auto}.split{display:block;min-height:0}.list{border-right:0;margin:0}.detail.open{inset:0 0 calc(66px + env(safe-area-inset-bottom));background:var(--panel)}.detail.open .detail-head{background:linear-gradient(135deg,#171c22,#111419 60%)}.detail.open .detail-body{padding-bottom:30px}.data-grid,.account-form,.finance-form{grid-template-columns:1fr}.account-form-submit .primary{width:100%;min-height:54px}.processor-hero{padding:22px 18px}.processor-hero h2{font-size:29px}.processor-card{grid-template-columns:28px minmax(0,1fr)}.processor-actions{grid-template-columns:1fr 1fr}.ops-toast{left:12px;right:12px;bottom:calc(78px + env(safe-area-inset-bottom))}.command-modal{padding-top:7vh}.command-foot{justify-content:space-between}.record-modal{padding:6px}}
...
<body>
  ...
  <script>
    ...
    function pendingApprovalMarkup(){var pending=state.jobs.filter(function(job){return job.status==="pending"&&!job.archivedAt;});if(!pending.length)return '<section class="processor-approval-queue"><div class="processor-approval-head"><div><span class="eyebrow">APPROVAL GATE</span><h3>Jobs waiting to be approved</h3></div><strong>0</strong></div><p class="processor-approval-empty">No jobs are waiting for approval.</p></section>';return '<section class="processor-approval-queue"><div class="processor-approval-head"><div><span class="eyebrow">APPROVAL GATE</span><h3>Jobs waiting to be approved</h3></div><strong>'+pending.length+'</strong></div><div class="processor-approval-list">'+pending.map(function(job){return '<article class="processor-approval-card"><div><div class="queue-status-cluster"><span class="reference">'+esc(job.reference)+'</span><span class="badge pending">PENDING</span></div><h4>'+esc(job.requesterName)+'</h4><p>'+esc(job.pickupAddress)+' → '+esc(job.primaryDropoffAddress)+'</p><p><strong>'+esc(job.serviceLevel)+'</strong> · '+money(job.totalCents)+'</p></div><div class="processor-approval-actions"><button type="button" class="secondary" data-processor-pending-job="'+esc(job.reference)+'">OPEN JOB</button><button type="button" class="primary queue-approve" data-processor-approve="'+esc(job.reference)+'">APPROVE</button></div></article>';}).join('')+'</div></section>';}
